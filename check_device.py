...
def check(challenge: str) -> None:

    print('\nChecking license request info')
    res = Session().post(
    ...
    print('Parsing device info\n')

    def match_reg(pattern: str, group: str | int) -> str:
    ...
    data = {
        'status': match_reg(r'Status.+\n.+\n.+>(.+)[<^]', 1),
        'systemId': system_id,
        'securityLevel': match_reg(r'Security Level.+\n.+\n.+>(.+)[<^]', 1),
